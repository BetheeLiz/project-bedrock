...
resource "aws_vpc" "project_bedrock_vpc" {
...
resource "aws_subnet" "public_subnets" {
  ...
  vpc_id                  = aws_vpc.project_bedrock_vpc.id
  cidr_block              = cidrsubnet(aws_vpc.project_bedrock_vpc.cidr_block, 8, count.index)
...
resource "aws_subnet" "private_subnets" {
  ...
  vpc_id            = aws_vpc.project_bedrock_vpc.id
  cidr_block        = cidrsubnet(aws_vpc.project_bedrock_vpc.cidr_block, 8, count.index + 10)
  availability_zone = data.aws_availability_zones.available.names[count.index]
